...
const pageInfo: Record<string, { name: string; title: string; description: string }> = {
  ...
  'leads': { name: 'All leads', title: 'Good people. Great possibilities.', description: 'Manage every registration, follow each connection, and keep your community moving.' },
...
function DashboardContent() {
  const { leads, activities, adminName, copy, exportLeads } = useApp()
  ...
  const [modal, setModal] = useState<'share' | 'help' | 'activity' | null>(null)
  ...
  function selectLead(lead: Lead) { setSelectedLeadId(lead.id) }
  function globalSearch(value: string) {
    setQuery(value)
    if (!['overview', 'leads', 'verification'].includes(currentTab)) {
  ...
  <main className="main-content"><section className="page-heading"><div><span className="page-eyebrow">{isOverview ? 'WORKSPACE OVERVIEW' : page.name.toUpperCase()}</span><h1>{page.title}</h1><p>{page.description}</p></div><div className="heading-actions"><div className="date-picker-container"><button className="button button-white date-button" onClick={() => setPopover(popover === 'date' ? null : 'date')} aria-expanded={popover === 'date'}><CalendarDays size={16} /><span>{period}</span><ChevronDown size={13} /></button>{popover === 'date' && <><div className="popover-scrim" onClick={() => setPopover(null)} /><div className="date-popover"><span className="popover-caption">REGISTRATION PERIOD</span>{['This month', 'Last 7 days', 'All time'].map(option => <button key={option} className={period === option ? 'selected' : ''} onClick={() => { setPeriod(option); setPopover(null) }}>{option}{period === option && <Check size={15} />}</button>)}<p>September 2026 demo workspace</p></div></>}</div><button className="button button-white export-button" onClick={() => exportLeads(leads.filter(lead => (period !== 'Last 7 days' || new Date(lead.date) >= new Date('2026-09-15')) && (!query || `${lead.name} ${lead.email} ${lead.code}`.toLowerCase().includes(query.toLowerCase()))))}><Download size={16} /><span>Export</span></button><button className="button button-primary invite-button" onClick={() => setModal('share')}><Plus size={17} /><span>Invite leads</span></button></div></section>
    <Metrics period={period} />
    {(isOverview || currentTab === 'leads' || currentTab === 'verification') && <LeadTable key={`${currentTab}-${period}`} onSelect={selectLead} query={query} onQueryChange={setQuery} period={period} initialStatus={currentTab === 'verification' ? 'pending' : 'all'} />}
    ...
    <footer className="workspace-footer"><span><span className="live-dot" />Demo workspace<span className="footer-divider">·</span>Changes saved on this device</span><span>Meaningful connections. Real possibilities.</span></footer>
  </main></div>
